...
//$(document).ready(function($) {
	...
	var url_base =  window.location.protocol+'//'+window.location.hostname+'/subdomains/beta/';  
   ...
   //poner el ancla	
	...
	$('body').on('click','.principal_menu', function (e) {   
		var catalogo= hash_url+'calendarios/calendarios';
    ...
    $('body').on('click','#guardar[value="si"]', function (e) {   
    	...
    	e.preventDefault(); 
    	    ...
    	    var id_session = $('#id_session').val();
 			...
 			var url = hash_url+'calendarios/anadir_carrito'; 
				$.ajax({
				    url: url,
				    method: "POST",
			        dataType: 'json',
					...
					success: function(todos){
			              $.each(todos.datos, function (i, valor) { 
						    	 ...
						    	 $.get(url_base+'?add-to-cart='+valor.id_diseno+'&quantity='+valor.cantidad+
									     	'&variation_id='+valor.id_tamano+'&attribute_pa_tamanos='+valor.nombre_tamano, function(resp) {
       								 console.log(resp);
 ...
 //////////////////////////////////////////////////////////////////////////
    ...
    var url = hash_url+'calendarios/activar_carrito'; 
				$.ajax({
				    url: url,
				    method: "POST",
			        dataType: 'json',
					...
					success: function(datos_eliminados){
							 ...
							 if (datos_eliminados['total_disenos'] == datos_eliminados['total']) {
							  		 $('#guardar').text('Agregar a Carrito');	 
							  		 $('#guardar').val('si');	 
	...
	$('body').on('click','.eliminar_slider', function (e) {   
       ...
       //Cuando cancela la "ELIMINACION DE UN TAMAÑO"
	    ...
	    $('body').on('click','#eliminar_diseno', function (e) {
		    ...
		    var url = hash_url+'calendarios/eliminar_diseno_revise'; 
				$.ajax({
				    url: url,
				    method: "POST",
			        dataType: 'json',
...
////////////////////////////////////////////////////////////////////////////////////////
	...
	$('body').on('click','.agregar_carrito[marcado="si"]', function (e) {   
		  ...
		  var nombre = $(this).attr('nombre');
 			...
 			$.get(url_base+'?add-to-cart='+id_diseno+'&quantity='+id_copia+
			 			...
			 			var url = hash_url+'calendarios/guardar_historico_informacion'; 
							$.ajax({
							    url: url,
							    method: "POST",
						        dataType: 'json',
	...
	$('body').on('click','.agrega_menu', function (e) {   
		...
		var $catalogo = hash_url+'calendarios/fotoimagen/'+$.base64.encode(id_session);

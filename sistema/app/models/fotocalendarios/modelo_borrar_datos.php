<?php if(! defined('BASEPATH')) exit('No tienes permiso para acceder a este archivo');
  class modelo_borrar_datos extends CI_Model{
    ...
    function __construct(){
      ...
      $this->historico_catalogo_logo    = $this->db->dbprefix('historico_catalogo_logo');
...
/*
historico_acceso
historico_catalogo_logo
historico_fotocalendario_lista
historico_lista_fechas_especiales
historico_lista_nombre_meses

-------------------------------------

historico_fechas_especiales
historico_fotocalendario_imagenes
historico_fotocalendario_imagenes_original
historico_fotocalendario_imagenes_recorte
historico_fotocalendario_temporal//se repite 2 veces
historico_logueo_identificador
historico_nombre_meses
*/
    ...
    public function completar_lista( $data ){
            ...
            if ( ($data['id_user']!=0) ){
                        $where = '(
                            ( ( id_session =  "'.$data['session_id'].'" ) OR
                              ( id_user =  "'.$data['id_user'].'" )
                            )
                            AND ( identificador_pedido =  0 )
                        )';   
            } else {
            ...
            $this->db->set( 'identificador_pedido', $data['identificador_pedido']);          
...
//nuevos
            ...
            $this->db->set( 'id_user', $data['id_user']);          
            $this->db->where($where);
            $this->db->update($this->historico_acceso );  


            $this->db->set( 'identificador_pedido', $data['identificador_pedido']);          
            $this->db->set( 'id_user', $data['id_user']);          
            $this->db->where($where);
            $this->db->update($this->historico_catalogo_logo );  

            $this->db->set( 'identificador_pedido', $data['identificador_pedido']);          
            $this->db->set( 'id_user', $data['id_user']);          
            $this->db->where($where);
            $this->db->update($this->historico_fotocalendario_lista );  

            $this->db->set( 'identificador_pedido', $data['identificador_pedido']);          
            $this->db->set( 'id_user', $data['id_user']);          
            $this->db->where($where);
            $this->db->update($this->historico_lista_fechas_especiales );  
            ...
            $this->db->set( 'id_user', $data['id_user']);          
            $this->db->where($where);
            $this->db->update($this->historico_lista_nombre_meses );  

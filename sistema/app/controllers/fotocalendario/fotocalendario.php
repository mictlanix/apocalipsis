...
//esto es temporal para borrar todos los datos
public function borrardatos() {
	
	print_r($this->modelo_borrar_datos->eliminar_diseno_completo());
}	

public function completar_lista() {
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	public function index($session) {
		...
		if ($data['datos']){
			...
			$this->load->view( 'sitio/fotocalendario/fotocalendario/seccion3', $data );
		} 	
	}
   ...
   echo json_encode($todo);   
	...
	function noguardar_lista() {
	          ...
	          $data['consecutivo']   =   $this->input->post('consecutivo');		          
 		 	  ...
 		 	  if (!empty($_FILES)) {
		          $config_adjunto['upload_path']    = './uploads/fotocalendario/fotocalendario/';
	...
	public function cargar_informacion(){ 
          ...
          if ($checar!=false) {
			...
			if ($logo!='') { //si hay imagen de logo 
			     ...
			     //$fichero    = './uploads/'.$modulo.'/img_'.$data['id_session'].'_'.$data['old_id_diseno'].'_'.$data['old_id_tamano'].'_'.$data['old_consecutivo'].$ext;
			     $fichero    = './uploads/'.$modulo.'/'.$logo;
			      if (file_exists($fichero)) {
						$nuevo_fichero    = './uploads/fotocalendario/fotocalendario/img_'.$data['id_session'].'_'.$data['id_diseno'].'_'.$data['id_tamano'].'_'.$data['consecutivo'].$ext;

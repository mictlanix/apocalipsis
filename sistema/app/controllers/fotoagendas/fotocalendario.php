...
class Fotocalendario extends CI_Controller {
	public function __construct(){ 
		...
		$this->load->model('fotoagendas/modelo_fotocalendario', 'modelo_fotocalendario'); 
...
//ok
public function eliminar_logo_fotoagendas() {
	...
	public function index($session) {
		...
		if ($data['datos']){
			   ...
			   $data['logos'] = $this->modelo_fotocalendario->listado_logos();		 	   
				...
				$this->load->view( 'sitio/fotoagendas/fotocalendario/seccion3', $data );
		} 	
	}
...
public function disenos_completos(){
    ...
    if ($data['datos'] != false)  {     
         foreach( (json_decode(json_encode($data['datos']))) as $clave =>$valor ) {
              array_push($cale_activo,array('variation_id' => $valor->variation_id,'id_diseno' => $valor->id_diseno,'consecutivo' => $valor->consecutivo, 'cantidad' => $valor->cantidad));  
       }
    } 
...
public function diseno_lista(){
				    ...
				    if ($dato['listas_dia'] != false)  {     
				         foreach( (json_decode(json_encode($dato['listas_dia']))) as $clave =>$valor ) {
				              array_push($list_dia,array('ano' => $valor->ano, 'mes' => $valor->mes, 'dia' => $valor->dia,'valor' => $valor->valor,
				              							'variation_id' => $valor->variation_id,'id_diseno' => $valor->id_diseno,'consecutivo' => $valor->consecutivo));  
	...
	function noguardar_lista() {
	          ...
	          $data['consecutivo']   =   $this->input->post('consecutivo');		          
 		 	  ...
 		 	  if (!empty($_FILES)) {
		          $config_adjunto['upload_path']    = './uploads/fotoagendas/fotocalendario/';
	...
	public function cargar_informacion(){ 
		  ...
		  $checar         = $this->modelo_fotocalendario->checar_existente_lista( $data );
		   //si existe ya registros borrarlos para crear nuevo		          
			...
			//--****-imagenes	
	    	  $eliminar          = $this->modelo_fotocalendario->eliminar_imagenes( $data );
	          $eliminar          = $this->modelo_fotocalendario->eliminar_imagenes_original( $data );
	          $eliminar          = $this->modelo_fotocalendario->eliminar_imagenes_recorte( $data );
		 ...
		 //--****-imagenes
			 $imagenes = $this->modelo_fotocalendario->leer_imagenes_original($data);
			 $imagenes = $this->modelo_fotocalendario->leer_imagenes_recorte($data);
			 $imagenes = $this->modelo_fotocalendario->leer_imagenes($data); 
			...
			if ($logo!='') { //si hay imagen de logo 
			     ...
			     $fichero    = './uploads/'.$modulo.'/'.$logo;
			      if (file_exists($fichero)) {
						$nuevo_fichero    = './uploads/fotoagendas/fotocalendario/img_'.$data['id_session'].'_'.$data['id_diseno'].'_'.$data['variation_id'].'_'.$data['consecutivo'].$ext;

...
class Fotocalendario extends CI_Controller {
	public function __construct(){ 
		parent::__construct();
		$this->load->model('libreta_corporativa/modelo_fotocalendario', 'modelo_fotocalendario'); 
	}
	...
	public function index($session) {
		...
		if ($data['datos']){
			   ...
			   $data['logos'] = $this->modelo_fotocalendario->listado_logos();		 	   
				...
				$this->load->view( 'sitio/libreta_corporativa/fotocalendario/seccion3', $data );
		} 	
	}
   ...
   echo json_encode($todo);   
	...
	function noguardar_lista() {
	          ...
	          $data['texto_pagina']   =   $this->input->post('texto_pagina');
 		 	  ...
 		 	  if (!empty($_FILES)) {
		          $config_adjunto['upload_path']    = './uploads/libreta_corporativa/fotocalendario/';
			  ...
			  $modulo = 'fotocalendario/fotocalendario'; 
	     	  ...
	     	  switch ($data['old_modulo']) {
		     	  	...
		     	  	case 'i':
		     	  		...
		     	  		break;
		     	  	case 'v':
		     	  		  $modulo = 'libreta_corporativa/fotocalendario';
		     	  		break;
			...
			if ($logo!='') { //si hay imagen de logo 
			     ...
			     $fichero    = './uploads/'.$modulo.'/'.$logo;
			      if (file_exists($fichero)) {
						$nuevo_fichero    = './uploads/libreta_corporativa/fotocalendario/img_'.$data['id_session'].'_'.$data['id_diseno'].'_'.$data['variation_id'].'_'.$data['consecutivo'].$ext;

...
class libreta_corporativa extends CI_Controller {
	public function __construct(){ 
		parent::__construct();
		$this->load->model('libreta_corporativa/modelo_fcalendario', 'modelo_fcalendario'); 
	}


    //comienza aqui a mostrar el formulario
	public function index($id_categ='libreta_corporativa',$pag='page', $cant=1){
			if ($id_categ=='page') {
				$cant=(int)$pag;
				$pag='page';
				$id_categ='libreta_corporativa';
			}
			$data['pagina'] =$cant;
			$data['id_categ'] =$id_categ;
			$this->load->view( 'sitio/libreta_corporativa/principal/principal',$data); 

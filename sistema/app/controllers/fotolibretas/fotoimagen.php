...
class Fotoimagen extends CI_Controller {
	public function __construct(){ 
		...
		$this->load->model('fotolibretas/modelo_fotoimagen', 'modelo_fotoimagen'); 
	...
	public function index($session) {
		...
		if (isset($_POST['id_diseno'])) {
				//$data['id_diseno']   = $_POST['variation_id']; // el diseño que se va activar
				$data['id_diseno']     = $_POST['id_diseno']; // el diseño que se va activar
				$data['variation_id']     = $_POST['variation_id']; // el diseño que se va activar
				$data['consecutivo']   = $_POST['consecutivo']; // el diseño que se va activar

				$data['ano']  		 = date("Y"); //$_POST['ano'];  //en que año
		...
		if  ($data['datos'] ) {
			$this->load->view( 'sitio/fotolibretas/fotoimagen/croppear',$data );	
			
			//$this->load->view( 'sitio/fotolibretas/fotoimagen/ancla',$data );	
	...
	public function buscarImagen(){
		...
		if ($data['datos']!=false) {
					...
					$variation_id = $data['datos']->variation_id;
					...
					$img_canva= '<div id="cont_img">';
							$img_canva .= '<img total_elementos="'.$total_elementos.'" id_diseno="'.$id_diseno.'" variation_id="'.$variation_id.'" consecutivo="'.$consecutivo.'"
		...
		$data['imagen'] = $img_canva.'<script src="'.base_url().'js/fotolibretas/fotoimagen/main.js" type="text/javascript"></script>';	
		    ...
		    mkdir($dir,0755,TRUE);  
 		 	  ...
 		 	  if (!empty($_FILES)) {
		          ...
		          $this->load->library('upload', $config_adjunto);
				   ...
				   echo '<script src="'.base_url().'js/fotolibretas/fotoimagen/main.js" type="text/javascript"></script>';
	}	

...
class Fotocalendario extends CI_Controller {
	public function __construct(){ 
		...
		$this->load->model('fotolibretas/modelo_fotocalendario', 'modelo_fotocalendario'); 
...
public function eliminar_logo_fotolibretas() {
	...
	public function index($session) {
		...
		if ($data['datos']){
			   ...
			   $data['logos'] = $this->modelo_fotocalendario->listado_logos();		 	   
				...
				$this->load->view( 'sitio/fotolibretas/fotocalendario/seccion3', $data );
		} 	
	}
...
*/
	...
	function noguardar_lista() {
	          ...
	          $data['consecutivo']   =   $this->input->post('consecutivo');		          
 		 	  ...
 		 	  if (!empty($_FILES)) {
		          $config_adjunto['upload_path']    = './uploads/fotolibretas/fotocalendario/';
			...
			if ($logo!='') { //si hay imagen de logo 
			     ...
			     $fichero    = './uploads/'.$modulo.'/'.$logo;
			      if (file_exists($fichero)) {
						$nuevo_fichero    = './uploads/fotolibretas/fotocalendario/img_'.$data['id_session'].'_'.$data['id_diseno'].'_'.$data['variation_id'].'_'.$data['consecutivo'].$ext;

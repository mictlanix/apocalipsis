...
class Fotocalendario extends CI_Controller {
	public function __construct(){ 
		...
		$this->load->model('agendas/modelo_fotocalendario', 'modelo_fotocalendario'); 
...
//ok
public function eliminar_logo_agendas() {
	...
	public function index($session) {
		...
		if ($data['datos']){
			   ...
			   $data['logos'] = $this->modelo_fotocalendario->listado_logos();		 	   
				...
				$this->load->view( 'sitio/agendas/fotocalendario/seccion3', $data );
		} 	
	}
...
public function diseno_lista(){
	...
	function noguardar_lista() {
	          ...
	          $data['consecutivo']   =   $this->input->post('consecutivo');		          
 		 	  ...
 		 	  if (!empty($_FILES)) {
		          $config_adjunto['upload_path']    = './uploads/agendas/fotocalendario/';
			...
			if ($logo!='') { //si hay imagen de logo 
			     ...
			     $fichero    = './uploads/'.$modulo.'/'.$logo;
			      if (file_exists($fichero)) {
						$nuevo_fichero    = './uploads/agendas/fotocalendario/img_'.$data['id_session'].'_'.$data['id_diseno'].'_'.$data['variation_id'].'_'.$data['consecutivo'].$ext;

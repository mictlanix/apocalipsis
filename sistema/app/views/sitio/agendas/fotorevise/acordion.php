...
<div class="col-md-12"> 
							...
							<div class="col-md-8 col-sm-9 col-xs-8">
	                            ...
	                            <div class="col-md-12 col-sm-12 text-right">  
	                                <form class="ac-custom ac-checkbox ac-checkmark" autocomplete="off">
	                                    <ul>
	                                        <li>
	                                            <input class="checando" id="check<?php echo $dato->id_diseno.$dato->variation_id.$dato->consecutivo; ?>" name="check<?php echo $dato->id_diseno.$dato->variation_id.$dato->consecutivo; ?>" type="checkbox"><label for="cb6">He revisado mis datos e información</label>
	                                        </li>
	                                    <ul>
	                                </form>                                      
	                            </div> 	                            
                            ...
                            <div class="row">
                                                     ...
                                                     <?php if ($fechas_especiales) {
                                                         echo '<h4 class="text-left">FECHAS ESPECIALES</h4>';
                                                          foreach ($fechas_especiales as $fechas) { 
                                                            if   (($dato->id_diseno==$fechas->id_diseno) and ($dato->variation_id==$fechas->variation_id)  and  ($dato->consecutivo==$fechas->consecutivo)  )   
                                                            { 
                                                                
                                                                  
                                                                  echo str_pad('<p><b>'.$fechas->dia, 2, "0", STR_PAD_LEFT).'/'.str_pad($fechas->mes, 2, "0", STR_PAD_LEFT).'/'.str_pad($fechas->ano, 4, "20", STR_PAD_LEFT).'</b>'.'  '.$fechas->valor.'</p>';
                                                              }
                                                          }}
                                                     ?>       
                								</div>
                                                     ...
                                                     <?php if ($nombre_meses) {
                                                         echo '<h4 class="text-left">MENSAJE DEL MES</h4>';
                                                          foreach ($nombre_meses as $mes) { 
                                                            if   (($dato->id_diseno==$mes->id_diseno) and ($dato->variation_id==$mes->variation_id)  and 
                                                              ($dato->consecutivo==$mes->consecutivo)  )   { 
                                                                
                                                                
                                                                echo '<p><b>'.$nom_mes[$mes->mes].'</b> '.$mes->valor.'</p>';
                               ...
                               <div class="col-md-3"  style="display:<?php echo ($informacion[0]->logo == "")  ? 'none': 'block'  ?>" >
                                    ...
                                    <?php if ($informacion[0]->logo != "") { ?>
                                        <img src="<?php echo base_url().'uploads/agendas/fotocalendario/'.$informacion[0]->logo; ?>" style="border:1px solid #c1c1c1; width:30%">
                                    <?php } ?>
  ...
  <!--panel panel-default miacordion desc"> -->
  <?php } }?>   
...
<div class="modal fade" id="modaleliminar_variacion" role="dialog" >  
  <div class="modal-dialog">
        <div class="modal-content">
            <?php $this->load->view( 'sitio/agendas/fotorevise/modaleliminar_variacion' ); ?>
        </div>
    </div>
</div>  

...
//****Define los parametros con los valores por defecto para la funcion "calendarioEventos"***
	$.fn.calendarioEventos.defaults = {
	    eventsjson: hash_url+'js/events.json',
		eventsLimit: 4,
		locales: {
			locale: "en",
...
//***Inicializar la funcion. Construye la funcion***
	...
	//cada comienzo de diseño en particular
			    ...
			    consecutivo = $("#consecutivo").val();
			 	...
			 	url =hash_url+"fotoagendas/diseno_lista";
	...
	$("body").on('change','#id_lista',function(e){		
					...
					correo_activo = jQuery('#correo_activo').val();
					id_lista = $(this).val();
					console.log(this);
					ubicacion_old = $('option:selected',this).attr('ubicacion'); 
					console.log(ubicacion_old);

				 	url =hash_url+"fotoagendas/leer_lista";
					$.ajax({
					    url: url,
					    type: 'POST',
					    dataType: "json",
	...
	function dibujarMes(movimiento, show, miCalendario, OpcionesEventos) {
		...
		var currentTime_mio = new Date()
		var month_mio = currentTime_mio.getMonth() + 1
		var day_mio = currentTime_mio.getDate()
		var year_mio = currentTime_mio.getFullYear()	
...
//fecha actual
			...
			fechaPosicionada = new Date(year_mio,0,1,0,0,0); // actual visible month
...
//Para imprimir los "días llenos"
		for (dayI = 1; dayI <= cantDiasMesMostrado; dayI++) {
			...
			listaDias.push('<li id="listaDia_' + dayI + '" rel="'+dayI+'" class="claseDia '+dayClass+'"><a href="#"  type="button" data-toggle="modal" data-target="#myModal"  >' + dayI + '<a href="javascript:void(0)"  dia=' + dayI + ' class="remover" tabindex="-1" title="retirar"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a> </a></li>');
		}

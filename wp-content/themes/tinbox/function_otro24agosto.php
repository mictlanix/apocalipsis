...
function probanddd( $cart_data, $cart_item = null ) {

      foreach ($cart_data as $key => $value) {
        // print_r($cart_item);
         //die;
         //print_r("v2:  ".$value["key"]);
  /*        print_r("v2:  ".$value["key"]);
          print_r("v3:  ".$value["value"]);
          echo '<br/><br/><br/>';
*/
          //$value["name"] ="osmel";
          //$custom_items = array();
           //$custom_items[] = array( "name" => 'etiqueta que sale en el carro '.$value["value"], "value" => $cart_item['campo_presentar'] );


//$custom_items[] = array( "name" => 'mi identificador'.$value["value"], "value" => $cart_item['attribute_pa_identificador'] );
//print_r($cart_item);

/*
ARRAY ( [PRODUCT_ID] => 664 [VARIATION_ID] => 665 [VARIATION] => ARRAY ( [ATTRIBUTE_PA_TAMANOS] => 6_POSTER [ATTRIBUTE_IDENTIFICADOR] => 112 ) [QUANTITY] => 1 [TM_EPO_PRODUCT_ORIGINAL_PRICE] => 25 [TM_EPO_OPTIONS_PRICES] => 0 [TM_EPO_PRODUCT_PRICE_WITH_OPTIONS] => 25 [LINE_TOTAL] => 25 [LINE_TAX] => 0 [LINE_SUBTOTAL] => 25 [LINE_SUBTOTAL_TAX] => 0 [LINE_TAX_DATA] => ARRAY ( [TOTAL] => ARRAY ( ) [SUBTOTAL] => ARRAY ( ) ) [TM_CART_ITEM_KEY] => 35355D151699EB506D1C5DE6F14FF8A7 [DATA] => WC_PRODUCT_VARIATION OBJECT ( [VARIATION_ID] => 665 [PARENT] => WC_PRODUCT_VARIABLE OBJECT ( [CHILDREN] => [PRICES_ARRAY:WC_PRODUCT_VARIABLE:PRIVATE] => ARRAY ( ) [ID] => 664 [POST] => WP_POST OBJECT ( [ID] => 664 [POST_AUTHOR] => 1 [POST_DATE] => 2016-04-26 13:47:24 [POST_DATE_GMT] => 2016-04-26 13:47:24 [POST_CONTENT] => [POST_TITLE] => 80'S [POST_EXCERPT] => [POST_STATUS] => PUBLISH [COMMENT_STATUS] => OPEN [PING_STATUS] => CLOSED [POST_PASSWORD] => [POST_NAME] => 80S-2 [TO_PING] => [PINGED] => [POST_MODIFIED] => 2016-04-26 13:51:00 [POST_MODIFIED_GMT] => 2016-04-26 13:51:00 [POST_CONTENT_FILTERED] => [POST_PARENT] => 0 [GUID] => HTTP://TINBOX.DEV.COM/TIENDA/CALENDARIOS/80S-2/ [MENU_ORDER] => 0 [POST_TYPE] => PRODUCT [POST_MIME_TYPE] => [COMMENT_COUNT] => 0 [FILTER] => RAW ) [PRODUCT_TYPE] => VARIABLE [SHIPPING_CLASS:PROTECTED] => [SHIPPING_CLASS_ID:PROTECTED] => 0 [TOTAL_STOCK] => [SUPPORTS:PROTECTED] => ARRAY ( ) [MANAGE_STOCK] => NO [TAX_STATUS] => TAXABLE ) [VARIATION_SHIPPING_CLASS] => [VARIATION_SHIPPING_CLASS_ID] => [VARIATION_HAS_SKU] => 1 [VARIATION_HAS_LENGTH] => 1 [VARIATION_HAS_WIDTH] => 1 [VARIATION_HAS_HEIGHT] => 1 [VARIATION_HAS_WEIGHT] => 1 [VARIATION_HAS_TAX_CLASS] => 1 
...
//add_filter( 'woocommerce_get_item_data', 'probanddd', 10, 2 );


//todos los hook de woocomerce 
//http://hookr.io/plugins/woocommerce/2.4.9/filters/#index=a&search=cart_item

//add_action( 'woocommerce_update_order_item', 'action_woocommerce_update_order_item', 10, 2 ); 
function action_woocommerce_update_order_item( $item_id, $args ) { 
    // make action magic happen here... 
  //print_r($item_id);
  //die;
}; 
//add_action( 'woocommerce_shop_loop_item_title', 'action_woocommerce_after_shop_loop_item_title' ); 
...
//add_action( 'woocommerce_cart_item_product', 'elementos');
function elementos(  $cart_item_data,  $cart_item,  $cart_item_key ) {
//    return "--".$cart_item_data.' * '.$cart_item.' * '.$cart_item_key;
}


//add_action( 'woocommerce_product_title', 'add_custom_name' );
function add_custom_name( $title ) {
    return "Nombre del producto :".$title;
}

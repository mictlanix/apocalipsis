...
elseif ( ( empty($_POST) && empty(TM_EPO()->cart_edit_key) )|| TM_EPO()->tm_epo_global_reset_options_after_add=="yes" ){
	$selected_value=-1;
}

$selected_value=apply_filters('wc_epo_default_value',$selected_value,isset($tm_element_settings)?$tm_element_settings:array());

$checked=false;

if($selected_value==-1){
	if ((empty($_POST) || TM_EPO()->tm_epo_global_reset_options_after_add=="yes")  && isset($default_value)){
		if ($default_value){
			$checked=true;
		}
	}

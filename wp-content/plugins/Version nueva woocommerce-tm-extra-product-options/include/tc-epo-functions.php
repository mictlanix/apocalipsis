...
if (!function_exists('tm_convert_local_numbers')){
...
if (!function_exists('tm_needs_wc_db_update')){
...
if (!tm_needs_wc_db_update() && !function_exists('wc_get_product') && get_option( 'woocommerce_db_version' )!==false && version_compare( get_option( 'woocommerce_db_version' ), '2.2', '<' ) ){
...
if (!function_exists('tm_fix_woocommerce_bundle_rate_shipping_scripts')){
...
function tc_epo_load_textdomain() {
	...
	if (file_exists($global_mo)){
		...
		load_plugin_textdomain( TM_EPO_TRANSLATION, false, TM_EPO_DIRECTORY . '/languages/' ); 
	}
}

...
final class TM_Extra_Product_Options {
	...
	public function __construct() {
		...
		$cart_edit_key_var 			= apply_filters('wc_epo_cart_edit_key_var','tm_cart_item_key');
		$cart_edit_key_var_alt 		= apply_filters('wc_epo_cart_edit_key_var_alt','tc_cart_edit_key');
			...
			if (is_array($epos) && (!empty($epos['global']) || !empty($epos['local']))) {
				if (!empty($epos['price'])){
					...
					if ($price ===''){
						...
						if (isset($minmax['min']) && $minmax['min']>=0){
	...
	public function tm_woocommerce_saved_order_items($order_id=0, $items=array() ){
		...
		if (is_array($items) && isset($items['tm_epo'])){
			...
			$order_currency = $order->get_order_currency();
			$mt_prefix = $order_currency;//TM_EPO_HELPER()->get_currency_price_prefix($order_currency);
					...
					foreach ($epos as $key => $epo) {
						if(isset($items['tm_item_id']) && isset($items['tm_key']) && $items['tm_key']==$key && $items['tm_item_id']==$item_id){
							...
							if(isset($epo['price'])){
								...
								if(!$new_currency){
									$epo['price'] = apply_filters('woocommerce_tm_epo_price2_remove',$epo['price']);
								}
			...
			if ($epos && is_array($epos) ){
				...
				foreach ($epos as $key => $epo) {
					if ($epo && is_array($epo)){
						...
						if (is_array($epo['value'])){
							$display_value = array_map('html_entity_decode', $display_value, ENT_COMPAT | ENT_HTML401 ,'UTF-8');
						}else{
							$display_value = html_entity_decode($display_value, ENT_COMPAT | ENT_HTML401 ,'UTF-8');
						}
		    		...
		    		foreach ( $tmcartfee as $cartfee ) {
				        ...
				        if (is_array($cart_object->fees )){    
							foreach ( $cart_object->fees as $fee ) {
								if ( $fee->id == sanitize_title($new_name) ) {
									$fee->amount=$fee->amount+(float) esc_attr( apply_filters( 'woocommerce_tm_epo_price2', apply_filters( 'woocommerce_tm_epo_price_on_cart',$new_price) ) );
									$canbadded=false;
									break;
								}
							}
						...
						if($canbadded){
							...
							if(isset($cartfee['price_per_currency']) && isset($cartfee['price_per_currency'][$to_currency]) && $cartfee['price_per_currency'][$to_currency]!=''){
								...
								$new_price = apply_filters( 'woocommerce_tm_epo_price2', apply_filters( 'woocommerce_tm_epo_price_on_cart',$new_price) );
							}					
	...
	public function add_cart_item( $cart_item=array() ) {
		...
		if ( ! empty( $cart_item['tmcartepo'] ) ) {
			...
			$cart_item['tm_epo_options_prices']=apply_filters('wc_epo_cart_options_prices',$tmcp_prices,$cart_item);

			$cart_item['tm_epo_product_original_price']=$cart_item['data']->price;

			if ( apply_filters( 'wc_epo_adjust_price', true, $cart_item ) ){
				$cart_item['data']->adjust_price( apply_filters('wc_epo_cart_options_prices',$tmcp_prices,$cart_item) );
			}
	...
	public function get_item_data_array( $other_data, $cart_item=array() ) {
		...
		if (is_array($filtered_array)){
			foreach ( $filtered_array as $section ) {
					...
					foreach ( $section['other_data'] as $key=>$data ) {
						...
						if (!empty($data['images']) && $this->tm_epo_show_image_replacement=="yes"){
							if ($this->tm_epo_hide_options_prices_in_cart=="normal"){
								...
								$new_price=apply_filters('wc_epo_get_RP_WCDPD', $cart_item['data'],$data['unit_price'],$cart_item[$this->cart_edit_key_var]);// $this->get_RP_WCDPD($cart_item['data'],$data['unit_price'],$cart_item['tm_cart_item_key']);
				...
				$section['price']=apply_filters('wc_epo_get_RP_WCDPD', $cart_item['data'],$section['price'],$cart_item[$this->cart_edit_key_var] );//$this->get_RP_WCDPD($cart_item['data'],$section['price'],$cart_item['tm_cart_item_key']);
				...
				if ($this->tm_epo_cart_field_display!="link"){
					if (empty($section['hidelabelincart']) || empty($section['hidevalueincart'])){
						$other_data[] = array(
							...
							'value'   			=> empty($section['hidevalueincart'])?do_shortcode(TM_EPO_HELPER()->html_entity_decode($value)). ((!$format_price_shown && $format_price && $quantity_string)?'<small>'.$format_price.$quantity_string . '</small>':(($format_price)?$format_price:'' )):'',
	...
	public function calculate_price( $post_data=null, $element, $key, $attribute, $per_product_pricing, $cpf_product_price, $variation_id,$price_default_value=0,$currency=false ) {
			...
			switch ($_price_type) {
				case 'percent':
					if ($cpf_product_price){
						...
						if ($currency){
							$cpf_product_price=apply_filters( 'wc_epo_get_currency_price',$currency,$cpf_product_price,"percent");
						}
				...
				case 'percentcurrenttotal':
					if ($_price!='' && isset($post_data[$attribute.'_hidden'])){
						...
						if ($currency){
							$_price=apply_filters( 'wc_epo_get_currency_price',$currency,$_price,"percentcurrenttotal");
						}
				...
				case 'char':
					...
					if ($currency){
						$_price=apply_filters( 'wc_epo_get_currency_price',$currency,$_price,$_price_type);
					}
					break;
				case 'charpercent':
					if ($cpf_product_price){
						$cpf_product_price=apply_filters( 'woocommerce_tm_epo_price2_remove',$cpf_product_price,"percent");
						if ($currency){
							$cpf_product_price=apply_filters( 'wc_epo_get_currency_price',$currency,$cpf_product_price,"charpercent");
						}
				...
				case 'charpercentnon':
					if ($cpf_product_price){
						...
						$cpf_product_price=apply_filters( 'woocommerce_tm_epo_price2_remove',$cpf_product_price,"percent");
						if ($currency){
							$cpf_product_price=apply_filters( 'wc_epo_get_currency_price',$currency,$cpf_product_price,"charpercentnon");
						}
						$_textlength=floatval(strlen(stripcslashes( utf8_decode($post_data[$attribute]) )))-$freechars;
						if ($_textlength<0){
							$_textlength=0;
				...
				case 'charpercentnonnospaces':
					if ($cpf_product_price){
						...
						$cpf_product_price=apply_filters( 'woocommerce_tm_epo_price2_remove',$cpf_product_price,"percent");
						if ($currency){
							$cpf_product_price=apply_filters( 'wc_epo_get_currency_price',$currency,$cpf_product_price,"charpercentnonnospaces");
						}
						$_textlength=floatval( strlen( preg_replace( "/\s+/", "", stripcslashes( utf8_decode( $post_data[$attribute] ) ) ) ) )-$freechars;
						if ($_textlength<0){
							$_textlength=0;
				...
				case 'charpercentnofirst':
					if ($cpf_product_price){
						$cpf_product_price=apply_filters( 'woocommerce_tm_epo_price2_remove',$cpf_product_price,"percent");
						if ($currency){
							$cpf_product_price=apply_filters( 'wc_epo_get_currency_price',$currency,$cpf_product_price,"charpercentnofirst");
						}
						$_textlength=floatval(strlen(stripcslashes( utf8_decode($post_data[$attribute]) )))-1;
						if ($_textlength<0){
							$_textlength=0;
					...
					foreach ($file_name as $key => $value) {
						if (!empty($value)){
							...
							if (!$check_filetype && !empty($file['name'])){
								$passed=false;
								wc_add_notice( __( "Sorry, this file type is not permitted for security reasons.", TM_EPO_TRANSLATION  ). ' ('.$check_filetype.')' , 'error' );
							}
						}
					}
				}else{
	...
	public function tm_check_match($posted_value,$value,$operator){
		$posted_value = rawurlencode(apply_filters('tm_translate',$posted_value));
	...
	public function custom_frontend_scripts() {
		...
		wp_register_script( 'tm-modernizr', TM_EPO_PLUGIN_URL. '/assets/js/modernizr.js', '', '2.8.2' );
		...
		wp_enqueue_script( 'tm-epo', TM_EPO_PLUGIN_URL. '/assets/js/tm-epo.js', array( 'jquery', 'wp-util', 'tm-owl-carousel','tm-datepicker', 'tm-accounting', 'tm-modernizr', 'tm-scripts' ), $this->version, true );
		...
		$args = array(
			...
			'i18n_characters_remaining' 			=> (!empty($this->tm_epo_characters_remaining_text))?$this->tm_epo_characters_remaining_text:__( 'characters remaining', TM_EPO_TRANSLATION ),
			...
			"tm_epo_global_validator_messages" 		=> array(
				"required" 	=> (!empty($this->tm_epo_this_field_is_required_text))?$this->tm_epo_this_field_is_required_text :__("This field is required.", TM_EPO_TRANSLATION ),
			...
			'dayNamesMin' 							=> $this->strip_array_indices( $wp_locale->weekday_initial ),
 			...
 			'wc_booking_person_qty_multiplier' 		=> ($this->tm_epo_bookings_person=="yes")?1:0,
        	'wc_booking_block_qty_multiplier' 		=> ($this->tm_epo_bookings_block=="yes")?1:0,
	...
	*/
	...
	private function print_price_fields( $product_id=0, $form_prefix="", $is_from_shortcode=false) {
		...
		if(class_exists('RP_WCDPD') && !empty($GLOBALS['RP_WCDPD'])){
			$price=apply_filters('wc_epo_get_RP_WCDPD', $product, NULL, NULL);//$this->get_RP_WCDPD($product);
			if ($price){
				$price['product']=array();
									...
									if ($this->tm_original_builder_elements[$current_element]){
										...
										if ($this->tm_original_builder_elements[$current_element]["type"]=="single" || $this->tm_original_builder_elements[$current_element]["type"]=="multipleallsingle"){
											...
											if (!$new_currency){
												...
												$_original_regular_price_filtered = apply_filters( 'woocommerce_tm_epo_price2', $_original_regular_price_filtered, $_for_filter_price_type );
											}
											...
											if ( isset( $builder['multiple_'.$current_element.'_options_price'][$current_counter] ) ) {
												...
												foreach ( $_prices as $_n=>$_price ) {
													...
													if(!$new_currency){
														$_price	= apply_filters( 'woocommerce_tm_epo_price2',$_price, $_for_filter_price_type);
														$_original_prices[$_n]= apply_filters( 'woocommerce_tm_epo_price2',$_original_prices[$_n], $_for_filter_price_type);
													}

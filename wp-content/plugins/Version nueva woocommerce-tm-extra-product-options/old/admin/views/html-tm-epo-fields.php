...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		if (isset($tm_nonce)){
			$messages[] = '<a href="' . esc_url( wp_nonce_url( "edit.php?post_type=product&page=".TM_EPO_GLOBAL_POST_TYPE_PAGE_HOOK."&doaction=undo&action=untrash&ids=$ids".$tm_bulk, $tm_nonce ) ) . '">' . __('Undo', TM_EPO_TRANSLATION) . '</a>';
		}
	}
}

...
/*        if ( has_post_thumbnail( $variation->get_variation_id() ) ) {
    ...
    public function row_actions( $actions, $post ){
        ...
        if( $can_do_clone ){
            ...
            $actions['tm_clone_form'] = '<a class="tm-clone-form" rel="'.$nonce.'" href="'.admin_url( "edit.php?post_type=product&amp;page=".TM_EPO_GLOBAL_POST_TYPE_PAGE_HOOK."&amp;action=clone&amp;post=".$post->ID."&amp;_wpnonce=".$nonce ).'">'.__( 'Clone form', TM_EPO_TRANSLATION ).'</a>';
        }

        // Export a form
        $nonce = wp_create_nonce( 'tmexport_form_nonce_'.$post->ID ); 
        $actions['tm_export_form'] = '<a class="tm-export-form" rel="'.$nonce.'" href="'.admin_url( "edit.php?post_type=product&amp;page=".TM_EPO_GLOBAL_POST_TYPE_PAGE_HOOK."&amp;action=export&amp;post=".$post->ID."&amp;_wpnonce=".$nonce ).'">'.__( 'Export form', TM_EPO_TRANSLATION ).'</a>';
        ksort($actions);
        return $actions;
    }
    ...
    public function admin_menu() {
        $page_hook = add_submenu_page( 'edit.php?post_type=product', __( 'TM Global Extra Product Options', TM_EPO_TRANSLATION ), __( 'TM Global Extra Product Options', TM_EPO_TRANSLATION ), 'manage_woocommerce', TM_EPO_GLOBAL_POST_TYPE_PAGE_HOOK, array( $this, 'admin_screen' ) );
    ...
    public function register_admin_scripts($override=0) {
        ...
        $params = array(
            ...
            'i18n_starts_with'          => __( 'starts with', TM_EPO_TRANSLATION ),
            'i18n_ends_with'            => __( 'ends with', TM_EPO_TRANSLATION ),
            'i18n_greater_than'         => __( 'greater than', TM_EPO_TRANSLATION ),
            'i18n_less_than'            => __( 'less than', TM_EPO_TRANSLATION ),
    ...
    public function tm_clone_form_action($original_id=0){        
        ...
        foreach ( $custom_fields as $key => $value ) {
            if ($key!=TM_EPO_WPML_LANG_META && $key!=TM_EPO_WPML_PARENT_POSTID){

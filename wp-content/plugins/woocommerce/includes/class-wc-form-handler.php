...
class WC_Form_Handler {
	...
	public static function eliminar_elemento_carrito($elemento) {
			//http://programacion.net/articulo/prevenir_ataques_inyeccion_sql_en_wordpress_358
			global $wpdb;

			$result = $wpdb->get_results($wpdb->prepare("SELECT * FROM tinbox_historico_logueo_identificador
			 WHERE id_old = %d", $elemento));			

		     $id_session = $result[0]->id_session;    
			 $id_tamano = $result[0]->id_tamano;
			 $id_diseno = $result[0]->id_diseno;
		   $consecutivo = $result[0]->consecutivo;
		        $modulo = $result[0]->modulo;


/*
      $this->db->delete( $this->tinbox_historico_fotocalendario_temporal, array( 'id_session' => $data['id_session'],  'id_tamano' => $data['id_tamano'],  'id_diseno' => $data['id_diseno'],   'consecutivo' => $data['consecutivo'], 'modulo' => $this->modulo ) );
*/

      		$array = array(
      			"tinbox_historico_fotocalendario_temporal",
				"tinbox_historico_acceso",
				"tinbox_historico_catalogo_logo",
				"tinbox_historico_fotocalendario_lista",
				"tinbox_historico_lista_fechas_especiales",
				"tinbox_historico_lista_nombre_meses",
				"tinbox_historico_fechas_especiales",
				"tinbox_historico_fotocalendario_imagenes",
				"tinbox_historico_fotocalendario_imagenes_original",
				"tinbox_historico_fotocalendario_imagenes_recorte",
				"tinbox_historico_nombre_meses"
      			);
      		//"tinbox_historico_logueo_identificador",
      		...
      		for ($i = 0; $i<count($array); $i++) {
					...
					$wpdb->query( 
						$wpdb->prepare( 
							"DELETE FROM ".$array[$i]."
							 WHERE id_session = %d AND 
								   id_tamano = %d AND 
								   id_diseno = %d AND 
								 consecutivo = %d AND 
								      modulo = %d 
							 ",$id_session,$id_tamano,$id_diseno,$consecutivo,$modulo)
					    );
			...
			$wpdb->query( 
				$wpdb->prepare( 
					"DELETE FROM tinbox_historico_logueo_identificador
					 WHERE id_old = %d", $elemento)
			    );
	...
	public static function update_cart_action() {
		...
		elseif ( ! empty( $_GET['remove_item'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'woocommerce-cart' ) ) {
			...
			if ( $cart_item = WC()->cart->get_cart_item( $cart_item_key ) ) {
				...
				self::eliminar_elemento_carrito($cart_item['variation']['attribute_identificador']);

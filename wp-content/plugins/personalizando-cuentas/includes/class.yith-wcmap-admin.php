...
if ( ! class_exists( 'YITH_WCMAP_Admin' ) ) {
	...
	class YITH_WCMAP_Admin {
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'yith_wcmap', YITH_WCMAP_ASSETS_URL . '/js/ywcmap-admin.js', array( 'jquery', 'jquery-ui-sortable' ), false, true );
		...
		public function update_wcmap_endpoints( $value, $option, $raw_value ) {
			...
			foreach( $endpoints as &$endpoint ) {
				...
				$options['slug']   = isset( $options['slug'] ) ? $this->create_endpoint_key( $options['slug'] ) : $endpoint;
				$options['content'] = stripslashes( $options['content'] );

				update_option( $option['id'] . '_' . $endpoint, $options );

				// synchronize wc options
				if( $endpoint == 'view-order' || $endpoint == 'edit-account' || $endpoint == 'edit-address' ) {
